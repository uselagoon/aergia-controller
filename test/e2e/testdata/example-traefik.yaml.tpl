apiVersion: v1
kind: Namespace
metadata:
  name: example-nginx
  labels:
    lagoon.sh/project: example
    lagoon.sh/environment: "nginx"
    lagoon.sh/environmentType: "development"
---
apiVersion: v1
kind: Service
metadata:
  name: example-nginx-svc
  namespace: example-nginx
  labels:
    lagoon.sh/service: "nginx"
    lagoon.sh/project: example
    lagoon.sh/environment: "nginx"
    lagoon.sh/environmentType: "development"
spec:
  ports:
  - port: 80
    targetPort: 8080
    protocol: TCP
    name: http
  selector:
    app: example-nginx
---
apiVersion: apps/v1
kind: Deployment
metadata:
  name: example-nginx
  namespace: example-nginx
  labels:
    idling.amazee.io/watch: 'true'
    idling.amazee.io/idled: 'true'
    lagoon.sh/service: "nginx"
    lagoon.sh/project: example
    lagoon.sh/environment: "nginx"
    lagoon.sh/environmentType: "development"
  annotations:
    idling.amazee.io/unidle-replicas: '3'
spec:
  replicas: 0
  selector:
    matchLabels:
      app: example-nginx
  template:
    metadata:
      labels:
        app: example-nginx
    spec:
      containers:
      - name: example-nginx
        image: nginxdemos/nginx-hello:plain-text
        ports:
        - containerPort: 8080
---
apiVersion: traefik.io/v1alpha1
kind: Middleware
metadata:
  annotations:
    lagoon.sh/branch: main
    lagoon.sh/version: v2.7.x
  labels:
    app.kubernetes.io/instance: traefik-middleware
    app.kubernetes.io/managed-by: build-deploy-tool
    app.kubernetes.io/name: aergia
    lagoon.sh/buildType: branch
    lagoon.sh/environment: main
    lagoon.sh/environmentType: production
    lagoon.sh/project: example-project
    lagoon.sh/template: traefik-middleware-0.1.0
  name: aergia
  namespace: example-nginx
spec:
  errors:
    query: /?namespace=example-nginx&url={url}
    service:
      name: aergia-controller-controller-manager-backend
      namespace: aergia-controller-system
      port: 80
    status:
    - "503"
---
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  name: example-ingress
  namespace: example-nginx
  # add annotation to expose the headers to the backend service for 503 only
  annotations:
    nginx.ingress.kubernetes.io/custom-http-errors: "503"
    traefik.ingress.kubernetes.io/router.middlewares: "example-nginx-aergia@kubernetescrd"
  labels:
    lagoon.sh/service: "nginx"
    lagoon.sh/project: example
    lagoon.sh/environment: "nginx"
    lagoon.sh/environmentType: "development"
    lagoon.sh/autogenerated: "true"
spec:
  rules:
  - host: aergia.${KIND_NODE_IP}.nip.io
    http:
      paths:
      - path: "/"
        pathType: Prefix
        backend:
          service:
            name: example-nginx-svc
            port:
              number: 80